...
export class AtualizaProdutoComponent implements OnInit {
  ...
  constructor(private _produtoService:ProdutoService, private _rounter: Router, private _activatedRoute:ActivatedRoute){
    ...
    this._produtoService.getProduto(this.produtoId).subscribe(
      (res: any) => {
        this.produto = new Produto(
          res[0].id, res[0].produto, res[0].descricao, res[0].foto, res[0].preco
        );
      }
    )
  }
